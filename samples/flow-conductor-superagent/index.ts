 ...
 *
 * This example demonstrates how to use flow-conductor with the superagent adapter
...
import type { SuperagentRequestConfig } from "@flow-conductor/adapter-superagent";

// Create a superagent adapter instance
...
async function example1_SimpleChain() {
  ...
  try {
    const result = await begin<Post, Response, SuperagentRequestConfig>(
      {
        config: {
          url: `${API_BASE}/posts/1`,
          method: "GET",
        },
        mapper: (result: Response) => result.body as Post,
      },
      adapter
    )
      .next({
        ...
        mapper: (result: Response) => result.body as Comment[],
      })
      .execute();
...
async function example2_MultiStepChain() {
  ...
  try {
    const result = await begin<User, Response, SuperagentRequestConfig>(
      {
        config: {
          url: `${API_BASE}/users/1`,
          method: "GET",
        },
        mapper: (result: Response) => result.body as User,
      },
      adapter
    )
      .next({
      ...
      .next({
        ...
        mapper: (result: Response) => result.body as Todo[],
      })
      .execute();
...
async function example4_ErrorHandling() {
  ...
  try {
    await begin<Post, Response, SuperagentRequestConfig>(
      {
        config: {
      ...
      .withErrorHandler((error) => {
        ...
        const superagentError = error as any;
        console.log(
          `Error status: ${superagentError.status || "Network Error"}`
        );
        console.log(
          `Error message: ${superagentError.message || String(error)}`
        );
      })
      .execute();
  } catch (error) {
    ...
    console.log(
      "Caught error:",
      superagentError.status || superagentError.message || String(error)
    );
  }
}
 ...
 */
async function main() {
  console.log("🚀 Flow-Conductor Superagent Adapter Examples");
  console.log(
