 ...
 *
 * This example demonstrates how to use flow-conductor with the axios adapter
...
import type { AxiosRequestConfigType } from "@flow-conductor/adapter-axios";

// Create an axios adapter instance
    ...
    >(
      {
        ...
        mapper: (result: AxiosResponse<Post>) => result.data
      },
      adapter
    )
      .next({
        ...
        mapper: (result: AxiosResponse<Comment[]>) => result.data,
      })
      .execute();
    ...
    >(
      {
        ...
        mapper: (result: AxiosResponse<User>) => result.data,
      },
      adapter
    )
      .next({
      ...
      .next({
        ...
        mapper: (result: AxiosResponse<Todo[]>) => result.data,
      })
      .execute();
...
async function example4_ErrorHandling() {
  ...
  try {
    await begin<Post, AxiosResponse, AxiosRequestConfigType>(
      {
        config: {
      ...
      .withErrorHandler((error) => {
        ...
        console.log(
          `Error status: ${axiosError.response?.status || "Network Error"}`
        );
 ...
 */
async function main() {
  console.log("🚀 Flow-Conductor Axios Adapter Examples");
  console.log(
