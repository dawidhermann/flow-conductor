...
**Report here**: [GitHub Security Advisory](https://github.com/dawidhermann/flow-pipe/security/advisories/new)
...
We appreciate your help in keeping flow-pipe secure!

## Security Features

### SSRF Protection

flow-pipe includes built-in protection against Server-Side Request Forgery (SSRF) attacks:
...
When a potentially dangerous URL is detected, flow-pipe throws an `SSRFError`:

```typescript
import { SSRFError } from 'flow-pipe';
...
When using flow-pipe server-side:
...
When using flow-pipe in browsers:
...
We recommend keeping flow-pipe up to date:

```bash
npm update flow-pipe
```
...
This security policy is part of the flow-pipe project and is subject to the same license terms.
